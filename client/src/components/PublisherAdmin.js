...
function PublisherAdmin({title,report,state1,timefinder,singlead,spentfinder,spentOffline,spentOfflined,spentOfflinev,colorfinder,ids,tablesorter,arrowRetuner}) {
                ...
                {singlead._id && adss.length && ids ?
                    <Table className={classes.table} aria-label="simple table">
                        <TableHead>
                            <TableRow>
                                <TableCell 
                                    onClick={()=>tablesorter('pub',1,'text','apppubidpo','publishername',true,setsa,setadss,adss)} 
                                    onDoubleClick={()=>tablesorter('revpub',-1,'text','apppubidpo','publishername',true,setsa,setadss,adss)}
                                >
                                    Publisher {arrowRetuner(sa==='pub' ? '1' : (sa ==='revpub' ? '2' : '3' ))}
                                </TableCell>
                                <TableCell>SSP {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell>Total Impressions to be delivered {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell 
                                    onClick={()=>tablesorter('impre',1,'num','impressions',false,false,setsa,setadss,adss)} 
                                    onDoubleClick={()=>tablesorter('revimpre',-1,'num','impressions',false,false,setsa,setadss,adss)}
                                >
                                    Total Impressions Delivered till date {arrowRetuner(sa==='impre' ? '1' : (sa ==='revimpre' ? '2' : '3' ))}
                                </TableCell>
                                <TableCell>Avg required {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell>Avg Achieved {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell>Total spent {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell>Total Clicks Delivered till date {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell>CTR {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell>Balance Impressions {arrowRetuner(sa==='cat' ? '1' : (sa ==='revcat' ? '2' : '3' ))}</TableCell>
                                <TableCell></TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
